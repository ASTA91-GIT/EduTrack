(() => {
  ...
  const kAttendance = (b, d, s, date) =>
    ns(
      `attendance:branch:${slug(b)}:division:${slug(d)}:subject:${slug(
        s
      )}:date:${date}`
    );
  ...
  function getBranches() {
    return load(kBranches, []);
  }
  function getDivisions(b) {
    return load(kDivs(b), []);
  }
  function getSubjects(b, d) {
    return load(kSubs(b, d), []);
  }
  function getStudents(b, d, s) {
    return load(kStud(b, d, s), []);
  }
  ...
  function renderStudents(branch, division, subject, date) {
    ...
    students.forEach((st) => {
      const status = existing[st.rollNo] || "absent";
      const tr = document.createElement("tr");
      tr.innerHTML = `<td>${st.rollNo}</td><td>${
        st.name
      }</td><td><button class="status-btn ${status}" data-roll="${st.rollNo}">${
  ...
  saveBtn.addEventListener("click", () => {
    ...
    tableBody.querySelectorAll(".status-btn").forEach((btn) => {
      result[btn.dataset.roll] = btn.classList.contains("present")
        ? "present"
        : "absent";
    });
